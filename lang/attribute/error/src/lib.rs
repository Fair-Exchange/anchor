...
/// Generates an [`Error::AnchorError`](../../safe_anchor_lang/error/enum.Error.html) that includes file and line information.
///
/// # Example
/// ```rust,ignore
/// #[program]
...
fn create_error(error_code: Expr, source: bool, account_name: Option<Expr>) -> TokenStream {
    let error_origin = match (source, account_name) {
        ...
        (false, Some(account_name)) => quote! {
            Some(safe_anchor_lang::error::ErrorOrigin::AccountName(#account_name.to_string()))
        },
        (true, _) => quote! {
            Some(safe_anchor_lang::error::ErrorOrigin::Source(safe_anchor_lang::error::Source {
                filename: file!(),
                line: line!()
            }))
        },
    ...
    TokenStream::from(quote! {
        safe_anchor_lang::error::Error::from(
            safe_anchor_lang::error::AnchorError {
