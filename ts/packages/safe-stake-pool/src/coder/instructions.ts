...
function encodeDepositSafe({ arg }: any): Buffer {
  return encodeData({ depositSafe: { arg } }, 1 + 8);
}
...
function encodeWithdrawSafe({ arg }: any): Buffer {
  return encodeData({ withdrawSafe: { arg } }, 1 + 8);
}
...
LAYOUT.addVariant(14, B.struct([B.u64("arg")]), "depositSafe");
LAYOUT.addVariant(
  15,
  B.struct([
    ((p: string) => {
...
LAYOUT.addVariant(16, B.struct([B.u64("arg")]), "withdrawSafe");
LAYOUT.addVariant(
  17,
