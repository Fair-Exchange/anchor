...
import { SafeTokenCoder } from "./coder";
...
export function splTokenProgram(params?: GetProgramParams): Program<SafeToken> {
  return new Program<SafeToken>(
    IDL,
    params?.programId ?? SAFE_TOKEN_PROGRAM_ID,
    params?.provider,
    new SafeTokenCoder(IDL)
  );
}
